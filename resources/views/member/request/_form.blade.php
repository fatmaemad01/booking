                        ...
                        @foreach($days as $day)
                            <li class="dropdown-item">
                                <div class="form-check">
                                    <input class="form-check-input" name="days[]" type="checkbox" value="{{$day->id}}"
                                        id="day-{{$day->id}}" @if($request->days->contains($day->id)) checked @endif>
                                    <label class="form-check-label" for="day-{{$day->id}}">
                                        {{$day->name}}
                                    </label>
                                </div>
                                <!-- Add start and end time fields for each day -->
                                <div class="row mt-2">
                                    <div class="col-6">
                                        <x-form.form-outline>
                                            <label class="form-label" for="start_time_{{$day->id}}">Start Time:</label>
                                            <x-form.input name="start_time_{{$day->id}}" id="start_time_{{$day->id}}"
                                                type="time" />
                                        </x-form.form-outline>
                                    </div>
                                    <div class="col-6">
                                        <x-form.form-outline>
                                            <label class="form-label" for="end_time_{{$day->id}}">End Time</label>
                                            <x-form.input name="end_time_{{$day->id}}" id="end_time_{{$day->id}}"
                                                type="time" />
                                        </x-form.form-outline>
                                    </div>
                                </div>
